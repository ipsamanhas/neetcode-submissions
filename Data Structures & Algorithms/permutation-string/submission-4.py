class Solution:
    def checkInclusion(self, s1: str, s2: str) -> bool:

        # check if a permutation of s1 is present in s2
        # return True if its present
        # return False if it isnt

        #check for the length first
        n1 = len(s1)
        n2 = len(s2)
 
        if n1 > n2:
            return False
 
        s1_counts = [0] * 26
        s2_counts = [0] * 26
 
        for i in range(n1):
            s1_counts[ord(s1[i]) - 97] += 1
            s2_counts[ord(s2[i]) - 97] += 1
 
        if s1_counts == s2_counts:
            return True
        
        # the window always stays size n1
        for i in range(n1, n2):
            s2_counts[ord(s2[i]) - 97] += 1 # add a new character -> window moves right
            s2_counts[ord(s2[i - n1]) - 97] -= 1 # remove the left most old character
            if s1_counts == s2_counts:
                return True
 
        return False

        
        